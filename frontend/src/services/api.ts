...
class ApiService {
  ...
  async sellTokens(amount: number, pricePerToken: number) {
    const response = await this.api.post('/tokens/sell', { amount, pricePerToken });
    return response.data;
  }
  ...
  async purchaseFromMarketplace(orderId: string) {
    const response = await this.api.post(`/tokens/marketplace/${orderId}/purchase`);
    return response.data;
  }
  ...
  async issueCredits(projectId: string, amount: number, verificationData: any) {
    const response = await this.api.post(`/admin/projects/${projectId}/issue-credits`, {
      amount,
      verificationData
    });
    return response.data;
